...
**弓を持つ手とは反対側の腕** を対象に、現在の姿勢を保ちながらIKへ切り替え、手のIKコントローラーと弓弦コントローラーを同時に選択します。
...
当初は手と弓弦を継続的にコンストレイントする方式も検討しました。

しかし、常に追従させる構造では弓を引いた後の細かな位置調整がしづらくなるほか、弓弦をリリースする際にコンストレイントの解除や状態管理が必要となり、処理が複雑になるという課題がありました。

そこで試行錯誤の結果、手のIKコントローラーと弓弦コントローラーを**必要なときだけ同時選択する方式**へ変更しました。

これにより、アニメーターが任意の位置から細かく調整できる操作自由度を保ちながら、不要なコンストレイントを増やさず、リリース処理もシンプルな構成にしています。

#### 実際の使用から操作フローを改善

初期実装では、ボタンを再度押した際にもPole Vectorを自動配置する仕様としていました。

しかし、実際にアニメーション作業で使用したところ、アニメーターが任意に調整した肘の位置まで再配置されてしまい、再調整が必要になることが分かりました。

そこで、Pole Vectorの自動配置を**初回のFK → IK切り替え時のみ**に限定しました。

2回目以降はFK / IKの切り替えやPole Vectorの再配置を行わず、調整したポーズを維持したまま、手IKと弓弦を再選択します。

これにより、選択が外れた場合でもボタンをもう一度押すだけで、**調整した肘位置を崩さず、そのまま弓弦操作を再開**できるようになりました。

IK側への初回の姿勢合わせでは、Shoulder・Elbow・Wristの位置関係からPole Vectorの位置を計算します。
